...
@Component({
    selector: 'getsubchildcategory',
    templateUrl: './GetSubChildcategory.html'
})

export class GetSubChildCategory {
  @Input() link1: string;
  routelink: string;
  ...
  ngOnInit() {
    if (this.link1 == undefined) {
      this.routelink = "AddSubChildCategory";
      this.editroutelink = "stationary/AddSubChildCategory"
      ...
      this.getdata = "2";
        ...
        localStorage.removeItem('editsubchildcategoryId');
    this.Componentservices
      .GetSubcategory("SubChildCategory", this.getdata).subscribe(
                data => {
    ...
    editSubCategory(SubCategory: SubCategory): void {
        localStorage.removeItem('editsubchildcategoryId');
        localStorage.setItem('editsubchildcategoryId', SubCategory.ID.toString());
